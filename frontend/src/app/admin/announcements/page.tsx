...
export default function AdminAnnouncements() {
  ...
  const fetchAnnouncements = async () => {
    try {
      const res = await fetch(`${API}/admin/annoncements`);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${API}/annoncement`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({
          title: form.title,
          status: form.status === "active", // string → boolean
        }),
      });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const token = sessionStorage.getItem("admin_token");
      await fetch(`${API}/admin/annoncement/${id}`, {
        method: "DELETE",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="bg-white rounded-2xl shadow-sm p-6">
          ...
          <form onSubmit={handleCreate} className="space-y-3">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-[#81190B] text-white py-2.5 rounded-xl font-semibold flex items-center justify-center gap-2 disabled:opacity-50 hover:bg-[#5a1008]"
            >
              <Plus size={16} />
              {loading ? "Adding..." : "Add"}
            </button>
        ...
        <div className="lg:col-span-2 space-y-3">
          {announcements.map((a) => (
            <div key={a._id} className="bg-white rounded-2xl shadow-sm p-4 flex items-center justify-between">
              <div>
                ...
                <span className={`text-xs font-semibold ${a.status ? "text-green-600" : "text-gray-400"}`}>
          ...
          {announcements.length === 0 && (
            <div className="bg-white rounded-2xl p-8 text-center text-gray-400">No announcements</div>
          )}
        </div>
      </div>
    </div>
