...
import { X, Plus, Minus, Trash2, ShoppingBag, Tag, ChevronRight, Gift, Truck } from 'lucide-react';
import Image from 'next/image';
import { useAppDispatch, useAppSelector } from '@/store/hooks';
import {
  closeCartSidebar, updateQuantity, removeItem,
  clearCoupon, applyPromoCode,
...
export default function CartSidebar({ onCheckout }: CartSidebarProps) {
  ...
  const { cartItems, sidebarOpen, promoApplied, discountAmount, promoCode, couponError, loading } = useAppSelector(s => s.cart);
  ...
  useEffect(() => {
    fetch(`${API}/settings`)
      .then(r => r.json())
      .then(d => {
        if (typeof d.data?.freeShippingThreshold === 'number') setFreeThreshold(d.data.freeShippingThreshold);
        if (typeof d.data?.shippingCost === 'number')          setShippingCost(d.data.shippingCost);
      })
      .catch(() => {})
  ...
  const handleApplyCoupon = () => {
    ...
    dispatch(applyPromoCode({ code: couponInput.trim().toUpperCase(), orderAmount: subtotal }));
  };
            ...
            <div className="px-5 py-3 bg-amber-50 border-b border-amber-100">
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <p className="text-xs text-amber-700 font-medium">
                  Add <span className="font-bold">₹{amountForFree.toFixed(0)}</span> more for{' '}
        ...
        <div className="flex-1 overflow-y-auto py-4 px-5 space-y-4">
          {cartItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full py-16 text-center">
              ...
              <p className="text-gray-400 text-sm mt-1">Add some delicious spices!</p>
              <button
                onClick={() => dispatch(closeCartSidebar())}
            ...
            cartItems.map(item => (
              <div key={item.product + (item.variantId || '')} className="flex gap-3">
                {/* Image */}
                <div className="relative w-20 h-20 flex-shrink-0 rounded-2xl overflow-hidden bg-gray-50 border border-gray-100">
                  {item.image
                    ? <Image src={item.image} alt={item.product_name} fill className="object-contain p-1" sizes="80px" />
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-gray-900 text-sm leading-snug line-clamp-2">{item.product_name}</p>
                      ...
                      <button
                        onClick={() => {
                          if (item.quantity <= 1) {
                            ...
                            dispatch(updateQuantity({ id: item.product, newQuantity: item.quantity - 1 }));
                          }
                        }}
                        className="w-5 h-5 flex items-center justify-center text-gray-600 hover:text-gray-900 transition-colors"
                      >
                        {item.quantity === 1 ? <Trash2 size={11} className="text-red-400" /> : <Minus size={12} />}
                      </button>
                      <span className="text-sm font-bold text-gray-900 min-w-5 text-center">{item.quantity}</span>
                      <button
                        onClick={() => dispatch(updateQuantity({ id: item.product, newQuantity: item.quantity + 1 }))}
                        className="w-5 h-5 flex items-center justify-center text-gray-600 hover:text-gray-900 transition-colors"
                      >
                        <Plus size={12} />
                      </button>
          ...
          <div className="border-t border-gray-100 px-5 pt-4 pb-5 space-y-4">
            ...
            {!promoApplied ? (
              ...
              <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-xl px-3 py-2">
                <span className="text-green-700 text-sm font-semibold">🎉 {promoCode} — ₹{discountAmount.toFixed(0)} off</span>
                <button
                  onClick={() => { dispatch(clearCoupon()); setCouponInput(''); }}
            ...
            <div className="space-y-1.5 text-sm">
              ...
              <div className="flex justify-between text-gray-500">
                ...
                <span className={shipping === 0 ? 'text-green-600 font-semibold' : ''}>
                  {!settingsLoaded ? '...' : shipping === 0 ? 'Free' : `₹${shipping}`}
                </span>
              </div>
              {promoApplied && (
