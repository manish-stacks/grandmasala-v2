...
import {
  X, MapPin, CreditCard, CheckCircle, Loader2, Plus, Truck,
  Mail, Phone, ArrowRight, RefreshCw, ChevronLeft, Tag,
  ShoppingBag, AlertCircle, User,
} from 'lucide-react';
      ...
      {steps.map((s, i) => (
        <React.Fragment key={s}>
          <div className={`flex items-center gap-1.5 ${i <= idx ? 'opacity-100' : 'opacity-30'}`}>
            ...
            <span className={`text-xs font-medium hidden sm:block ${i === idx ? 'text-[#81190B]' : 'text-gray-400'}`}>{labels[i]}</span>
          </div>
          {i < steps.length - 1 && (
            <div className={`flex-1 h-0.5 mx-1 rounded ${i < idx ? 'bg-green-500' : 'bg-gray-200'}`} style={{ minWidth: 16 }} />
          )}
        </React.Fragment>
      ))}
    </div>
...
export default function CheckoutModal({ onClose }: CheckoutModalProps) {
  ...
  const [addrForm, setAddrForm] = useState({
    name: '', addressLine: '', city: '', state: '', postCode: '', addressType: 'Home',
  });
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      if (!data.success) { setError(data.message || 'OTP bhejne mein dikkat aayi'); return; }
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        otpRefs.current[0]?.focus(); toast.success('OTP dobara bhej diya!');
  ...
  const handlePlaceOrder = async () => {
    ...
    if (!selectedAddr) { setError('Please select an address'); return; }
        ...
        const rzp = new (window as any).Razorpay({
          ...
          handler: async (resp: any) => {
            try {
              ...
              if (vData.success) {
                dispatch(clearCart()); onClose();
                router.push(vData.redirectUrl || '/order-success');
  ...
  // RENDER
  // ─────────────────────────────────────────────────────────────
  const showBackBtn = (step === 'review') || (step === 'address' && !alreadyLoggedIn);
      ...
      >
        ...
        <div className="relative bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-xl shadow-2xl flex flex-col max-h-[95dvh] sm:max-h-[90vh] overflow-hidden py-6">
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 flex-shrink-0">
            <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              <div className="min-w-0 flex-1">
                <h2 className="font-bold text-gray-900 text-sm leading-tight">Checkout</h2>
                <div className="mt-1">
                  <StepDots step={step} />
                </div>
              </div>
            ...
            <div className="p-4 space-y-4">
                  ...
                  {authMode === 'otp' && (
                    <div className="space-y-4">
                      <div className="bg-blue-50 border border-blue-100 rounded-2xl p-3 text-center">
                        <p className="text-sm text-blue-700 font-medium">{resolvedEmail}</p>
                        <p className="text-xs text-blue-400 mt-0.5">Check inbox or spam folder</p>
                      </div>

                      <OtpBoxes
                        otp={otp} onChange={handleOtpChange}
                        onKeyDown={handleOtpKeyDown} refs={otpRefs} onPaste={handleOtpPaste}
                      />
                      ...
                      <div className="flex items-center justify-between text-sm">
                        <button
                          onClick={() => { setAuthMode('identifier'); setOtp(['', '', '', '', '', '']); setError(''); }}
              ...
              {step === 'address' && (
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    {!promoApplied ? (
                      ...
                      <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-xl px-3 py-2">
                        ...
                        <button onClick={() => { dispatch(clearCoupon()); setCouponInput(''); }} className="text-red-400 hover:text-red-600">
                          <X size={13} />
                        </button>
                      </div>
                    )}
                  ...
                  {/* Shipping info */}
                  <div className={`flex items-center justify-between p-3 rounded-xl border-2 ${shipFee === 0 ? 'border-green-400 bg-green-50/40' : 'border-gray-200'}`}>
                    <div className="flex items-center gap-2">
                      <Truck size={15} className={shipFee === 0 ? 'text-green-600' : 'text-gray-500'} />
                      <span className="text-sm text-gray-700 font-medium">Shipping</span>
                    </div>
                    <div className="text-right">
                      <span className={`font-bold text-sm ${shipFee === 0 ? 'text-green-600' : 'text-gray-900'}`}>
                        ...
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              <span className="font-bold text-gray-900 text-sm">{savedAddr.name}</span>
                              <span className="text-[10px] bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded-full uppercase">{savedAddr.addressType}</span>
                            </div>
                            <p className="text-xs text-gray-500">{savedAddr.addressLine}, {savedAddr.city}, {savedAddr.state} — {savedAddr.postCode}</p>
                            {savedAddr.mobile && <p className="text-xs text-gray-400 mt-0.5">📱 {savedAddr.mobile}</p>}
                          </div>
                          {selectedAddr === savedAddr && <CheckCircle size={18} className="text-[#81190B] flex-shrink-0" />}
                        </div>
                      </button>
                    )}
                      ...
                      <div className="space-y-2.5">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <input value={addrForm.city} onChange={e => setAddrForm(p => ({ ...p, city: e.target.value }))}
                            placeholder="City *" className="border-2 border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#81190B]" />
                          <input value={addrForm.state} onChange={e => setAddrForm(p => ({ ...p, state: e.target.value }))}
                            placeholder="State *" className="border-2 border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#81190B]" />
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                          <input value={addrForm.postCode} onChange={e => setAddrForm(p => ({ ...p, postCode: e.target.value }))}
                            placeholder="PIN Code *" className="border-2 border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#81190B]" />
                        ...
                        {savedAddr && (
                          <button onClick={() => setAddingNew(false)} className="text-xs text-gray-400 hover:text-gray-600">Cancel</button>
                        )}
                      </div>
                    )}
                  </div>
              ...
              {step === 'review' && (
                <div className="space-y-4">
                    ...
                    <div className="bg-blue-50 border border-blue-100 rounded-2xl p-3.5">
                      <div className="flex items-start justify-between">
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            <p className="text-[10px] font-bold text-blue-600 uppercase tracking-wide mb-1">Delivering to</p>
                            <p className="font-semibold text-gray-900 text-sm">{selectedAddr.name}</p>
                            <p className="text-xs text-gray-500">{selectedAddr.addressLine}, {selectedAddr.city} — {selectedAddr.postCode}</p>
                            <p className="text-xs text-gray-400">{selectedAddr.state}, India</p>
                          </div>
                        </div>
                        <button onClick={() => { setStep('address'); setError(''); }} className="text-xs text-blue-600 hover:underline flex-shrink-0">Change</button>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="grid grid-cols-2 gap-2.5">
                      {[
                        ...
                        <button key={opt.id} onClick={() => setPaymentMethod(opt.id)}
                          ...
                          <p className={`font-bold text-xs ${paymentMethod === opt.id ? 'text-[#81190B]' : 'text-gray-900'}`}>{opt.title}</p>
                  ...
                  <div className="bg-gray-50 rounded-2xl p-3.5">
                    ...
                    <div className="space-y-2.5">
                      {cartItems.map(item => (
                        <div key={item.product} className="flex items-center gap-2.5">
                          <div className="relative w-10 h-10 rounded-lg overflow-hidden bg-white flex-shrink-0 border border-gray-100">
                            {item.image
                              ? <Image src={item.image} alt={item.product_name} fill className="object-contain" sizes="40px" />
                          ...
                          <p className="text-xs font-bold text-gray-900 flex-shrink-0">₹{(item.price * item.quantity).toFixed(0)}</p>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="mt-3 pt-3 border-t border-gray-200 space-y-1.5 text-xs">
                      ...
                      {promoApplied && (
                        <div className="flex justify-between text-green-600"><span>Discount ({promoCode})</span><span>-₹{discount.toFixed(0)}</span></div>
                      )}
          ...
          <div className="px-4 pb-5 pt-3 border-t border-gray-100 flex-shrink-0">
            ...
            <div className={`flex gap-2 ${showBackBtn ? 'flex-row' : 'flex-col'}`}>
              ...
              {step === 'auth' && authMode === 'identifier' && (
                <button onClick={handleSendOtp} disabled={authLoading}
                  ...
                  {authLoading ? <><RefreshCw size={16} className="animate-spin" /> Sending...</> : <><ArrowRight size={16} /> Send OTP</>}
                </button>
              )}

              {step === 'auth' && authMode === 'otp' && (
                <button onClick={handleVerifyOtp} disabled={authLoading || otp.join('').length < 6}
                  className="flex-1 bg-[#81190B] hover:bg-[#5a1008] text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2 disabled:opacity-50 transition-colors shadow-lg shadow-red-900/15">
                  {authLoading ? <><RefreshCw size={16} className="animate-spin" /> Verifying...</> : <><CheckCircle size={16} /> Verify & Continue</>}
                </button>
              )}
